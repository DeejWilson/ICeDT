...
#-------------------------------------------------------------------#
lmInit_noWgt_suppRef<-function(yr,Zm,Zt){
  eta = yr[1]
  y   = yr[-c(1)]
...
#-------------------------------------------------------------------#
AbProf_Init_noWgt_suppRef <- function(x,Z,nG,Qval){
  Y      = x[c(1:nG)]
  logY   = log(Y)
  ...
  init_val[1] = HS_Sigma2_Update_noWgt_suppRef(logY = logY[-c(g2use)],eta_ij = eta_ij[-c(g2use)],
                                 EM_wgt = rep(1,(nG-length(g2use))),AB_Up = FALSE)
  init_val[2] = HS_Sigma2_Update_noWgt_suppRef(logY = logY[c(g2use)],eta_ij = eta_ij[c(g2use)],
