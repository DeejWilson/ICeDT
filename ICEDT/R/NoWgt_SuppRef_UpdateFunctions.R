...
HS_GradFunc_noFix_noWgt_suppRef <- function(x,logY,rho_i0,Z,Z_star,Sigma2,Sigma2A,EM_wgt){
  rho = c(0,x)
...
HS_GradFunc_Fix_noWgt_suppRef <- function(x,logY,rho_i0,Z,Z_star,Sigma2,Sigma2A,EM_wgt){
...
#-------------------------- Sigma Updates --------------------------#
HS_Sigma2_Update_noWgt_suppRef<-function(logY,eta_ij,EM_wgt,AB_Up=FALSE){
...
#----------------- Likelihood (No Fixed) ----------------------#
hin_func_noFix_noWgt_suppRef<-function(x,rho_i0,...){
  return(c(x-0.005,1-sum(x)-0.005))
}

hin_jacob_noFix_noWgt_suppRef <-function(x,rho_i0,...){
  return(rbind(diag(1,length(x)),rep(-1,length(x))))
}

HS_ComputeLik_noFix_noWgt_suppRef<-function(x,logY,rho_i0,Z,Z_star,Sigma2,Sigma2A,EM_wgt){
  rho  = c(0,x)
...
#----------------- Likelihood (No Fixed) ----------------------#
hin_func_Fix_noWgt_suppRef<-function(x,rho_i0,...){
  return(c(x-0.005,1-rho_i0-sum(x)-0.005))
}

hin_jacob_Fix_noWgt_suppRef <-function(x,rho_i0,...){
  return(rbind(diag(1,length(x)),rep(-1,length(x))))
}

HS_ComputeLik_Fix_noWgt_suppRef<-function(x,logY,rho_i0,Z,Z_star,Sigma2,Sigma2A,EM_wgt){
...
HS_UpdatePropn_Single_noWgt_suppRef <- function(x,Z,nCell,nG,maxIter_prop,useRho){
  #----------------------------------------#
  # Extract Info                           #
  #----------------------------------------#
  rho_i0 = x[1]
  ...
  theta_ij = log(eta_ij)
  Sigma2M_1 = HS_Sigma2_Update_noWgt_suppRef(logY   = logY,
                                             eta_ij = eta_ij,
                                             EM_wgt = EM_wgt,AB_Up = FALSE)
  Sigma2A_1 = HS_Sigma2_Update_noWgt_suppRef(logY   = logY,
                                             eta_ij = eta_ij,
  ...
  for(k in 1:maxIter_prop){
    ...
    #----------------------------------#
    if(useRho){
      auglagOut = auglag(par = urho_0[-c(nCell)],fn = HS_ComputeLik_Fix_noWgt_suppRef,
                         gr = HS_GradFunc_Fix_noWgt_suppRef,hin = hin_func_Fix_noWgt_suppRef,
                         hin.jac = hin_jacob_Fix_noWgt_suppRef,
      ...
      auglagOut = auglag(par = urho_0,fn = HS_ComputeLik_noFix_noWgt_suppRef,
                         gr = HS_GradFunc_noFix_noWgt_suppRef,hin = hin_func_noFix_noWgt_suppRef,
                         hin.jac = hin_jacob_noFix_noWgt_suppRef,
                         logY = logY,rho_i0=rho_i0,Z=Z,Z_star=Z_star,Sigma2 = Sigma2M_0,
                         Sigma2A = Sigma2A_0,EM_wgt=EM_wgt,
                         control.optim=list(fnscale=-1),control.outer = list(trace=FALSE))
...
HS_UpdatePropn_All_noWgt_suppRef<-function(logY,Rho_init,fixed_rho,Z,maxIter_prop,EM_wgt,useRho){
  PropnInfo = rbind(fixed_rho,Rho_init,logY,EM_wgt)
  out = apply(X=PropnInfo,MARGIN = 2,FUN = HS_UpdatePropn_Single_noWgt_suppRef,Z=Z,useRho = useRho,
...
#|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||#
#-------------------------------------------------------------------#
HS2_UpdateWgts_Single_noWgt_suppRef <-function(x,Z,nG,Qval){
  #-----------------------------------#
  # Extract Information               #
  #-----------------------------------#
  logY      = x[c(1:nG)]
...
HS2_UpdateWgts_All_noWgt_suppRef <- function(logY,Rho_init,fixed_rho,Sigma2M,Sigma2A,Z,p_m){
  Wgt_Info = rbind(logY,Rho_init,fixed_rho,Sigma2M,Sigma2A,p_m)
  
  EM_wgt = apply(X = Wgt_Info,MARGIN = 2,FUN = HS2_UpdateWgts_Single_noWgt_suppRef,
                 nG = nrow(logY),Qval = nrow(Rho_init),Z=Z)
  
  return(EM_wgt)
}

RP_UpdatePM_noWgt_suppRef <- function(nG,EM_wgt){
